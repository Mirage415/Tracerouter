...
class Traceroute:
    ...
    def _get_default_source_ip(self) -> str:
        """Get default source IP based on destination IP family"""
        try:
    ...
    def _print_warning(text: str):
        """Print warning message with red color"""
        color = "\033[91m"
        reset_color = "\033[0m"
        print(f"{color}{text}{reset_color}")

    def _decide_ip_protocol(self) -> str:
        """Determine IP protocol version based on destination IP"""
        if self.ip_protocol:
    ...
    def _init_sockets(self):
        """Initialize network sockets based on configuration"""
        try:
    ...
    def _create_recv_socket(self) -> socket.socket:
        """Create and configure the receiving socket"""
        try:
            family = socket.AF_INET6 if self.ip_protocol == '6' else socket.AF_INET
    ...
    def _config_socket_options(self):
        """Configure socket options based on traceroute parameters"""
        # Set TOS/DSCP
    ...
    def _config_source_routing(self):
        """Configure source routing options"""
        if self.ip_protocol == '4':
    ...
    def _build_ipv4_header(self) -> bytes:
        """Build IPv4 header for raw packets"""
    ...
    def _build_ipv6_header(self, payload_len: int = 40) -> bytes:
        """Build IPv6 header for raw packets"""
        version = 0x6
    ...
    def _build_ipv6_routing_header(self) -> bytes:
        """Build IPv6 routing header (type 0)"""
        segments = len(self.LSRR_list)
    ...
    def _match_reply(self, probe_id: Optional[int], seq: int, src_ip: str, extensions: dict):
        """Match reply to sent probe and record result"""
        with self.lock:
    ...
    def _display_current_hop(self, ttl: int):
        ...
        # Find first responding IP for this hop
        responding_ip = None
        for proto in ['udp', 'tcp', 'icmp']:
        ...
        # Display RTTs for each protocol
        for proto in self.probe_sequence:
    ...
    def _display_final_results(self):
        """Display final traceroute results"""
        print("\n=== Traceroute Results ===")
    ...
    def _process_icmp_extensions(packet: bytes) -> dict:
        """Process ICMP extensions in packet"""
        extensions = {
            'mpls': [],
            'other': []
        }
